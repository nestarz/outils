...
import type { FreshContext } from "../../createRenderPipe.ts";
...
export type SqliteMiddleware<Schema, Namespace extends string = "default"> = {
  handler: (
    _req: Request,
    ctx: FreshContext<SqliteMiddlewareState<Schema, Namespace>>,
  ) => Promise<Response>;
};

export const createSqliteMiddleware = <
  Schema,
  Namespace extends string = "default",
>({
  database,
  ...
  namespace?: Namespace;
}): SqliteMiddleware<Schema, Namespace> => {
  ...
  return {
    handler: async (
      ...
      ctx.state.clientQuery ??= {} as Record<Namespace, QueryFunction<Schema>>;
