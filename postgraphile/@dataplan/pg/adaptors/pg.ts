...
import type {
  ...
  PgClientResult,
  WithPgClient,
} from "@dataplan/pg/executor";
import type { MakePgServiceOptions } from "@dataplan/pg/interfaces";

// Set `DATAPLAN_PG_PREPARED_STATEMENT_CACHE_SIZE=0` to disable prepared statements
const cacheSizeFromEnv =
  Deno.env.get("DATAPLAN_PG_PREPARED_STATEMENT_CACHE_SIZE")
    ? parseInt(Deno.env.get("DATAPLAN_PG_PREPARED_STATEMENT_CACHE_SIZE"), 10)
    : null;
/**
 ...
 */
const DONT_DISABLE_JIT = Deno.env.get("DATAPLAN_PG_DONT_DISABLE_JIT") === "1";
...
function newNodePostgresPgClient(
  ...
  return {
    ...
    withTransaction(callback) {
      ...
      return addToQueue(async () => {
        if (txLevel === 0 && !alreadyInTransaction) {
          await pgClient.queryObject({ text: "begin" });
        } else {
        ...
        try {
          ...
          if (txLevel === 0 && !alreadyInTransaction) {
            await pgClient.queryObject({ text: "commit" });
          } else {
            await pgClient.queryObject({
          ...
          try {
            if (txLevel === 0 && !alreadyInTransaction) {
              await pgClient.queryObject({ text: "rollback" });
            } else {
              await pgClient.queryObject({
    ...
    query<TData>(opts: PgClientQuery): Promise<PgClientResult<TData>> {
      ...
      function doIt() {
        const { text, name, args, arrayMode } = opts;
              ...
              connection._graphilePreparedStatementCache = new LRU({
                ...
                dispose(key) {
                  if (connection.parsedStatements[key]) {
                    pgClient
                      .queryObject(
                        `deallocate ${pgClient.escapeIdentifier(key)}`,
                      )
                      .then(() => {
        ...
        return pgClient.queryObject(queryObj);
      }
    },
  };
}
      ...
      if (pgSettingsEntries.length > 0) {
        ...
        try {
          await pgClient.queryObject({
            ...
            args: [JSON.stringify(pgSettingsEntries)],
          });
  ...
  const withPgClient: WithPgClient<NodePostgresPgClient> = async (
    ...
    if (!pgClient[$$isSetup]) {
      ...
      if (!DONT_DISABLE_JIT) {
        ...
        pgClient.queryArray("set jit_optimize_above_cost = -1;").catch((e) => {
          console.error(
...
> implements GrafastSubscriber<TTopics> {
  ...
  private async syncWithClient(client: PoolClient) {
    ...
    for (const topic of topicsToAdd) {
      await client.queryObject(`LISTEN ${client.escapeIdentifier(topic)}`);
      this.subscribedTopics.add(topic);
    }
    for (const topic of topicsToRemove) {
      await client.queryObject(`UNLISTEN ${client.escapeIdentifier(topic)}`);
        ...
        try {
          for (const topic of this.subscribedTopics) {
            await client.queryObject(
              `UNLISTEN ${client.escapeIdentifier(topic)}`,
            );
            this.subscribedTopics.delete(topic);
          }
        } catch (e) {
...
export function makePgService(
  ...
  const pool: Pool = options.pool ?? new pg.Pool(connectionString, 5);
  if (!options.pool) {
  ...
  if (pubsub) throw Error("Not implemented");
  const pgSubscriber = options.pgSubscriber ??
    (pubsub ? /* new PgSubscriber(pool) */ null : null);
