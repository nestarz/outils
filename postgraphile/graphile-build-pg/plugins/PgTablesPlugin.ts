...
import { addBehaviorToTags } from "../utils.ts";
  ...
  inflection: {
    add: {
      _schemaPrefix(options, { pgNamespace, serviceName }) {
        ...
        const databasePrefix = serviceName === pgService?.name
          ? ""
          : `${serviceName}_`;
  ...
  gather: gatherConfig({
    ...
    helpers: {
      getResourceOptions(info, serviceName, pgClass) {
        let resourceOptionsByPgClass = info.state
          .resourceOptionsByPgClassByService.get(serviceName);
        ...
        resourceOptions = (async () => {
          ...
          const attributes = await info.helpers.pgIntrospection
          ...
          const directConstraints = await info.helpers.pgIntrospection
            .getConstraintsForClass(
              serviceName,
              pgClass._id,
            );

          const inheritance = await info.helpers.pgIntrospection
          ...
          const uniques = await Promise.all(
            uniqueAttributeOnlyConstraints.map(async (pgConstraint) => {
              const { tags, description } = pgConstraint
          ...
          const identifier =
            `${serviceName}.${namespace.nspname}.${pgClass.relname}`;
          ...
          const registryBuilder = await info.helpers.pgRegistry
            .getRegistryBuilder();
